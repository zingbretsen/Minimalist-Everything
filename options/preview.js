...
function imagePreview() {
  const xOffset = 0,
    yOffset = 32;
  $("#page-options")
    .on("mouseout", "label[screen]", () => {
      $("#hoverPreview").remove();
    })
    .on("mouseover", "label[screen]", (evt) => {
      const $self = $(evt.currentTarget),
        xPos = $self.offset().left,
        yPos = $self.offset().top;
      $(document.body).append(`<div id="hoverPreview"><img src="${$self.attr("screen")}" /></div>`);
      $("#hoverPreview img").load(() => {
        $("#hoverPreview img").css({
        ...
        if ($("#hoverPreview").height() > (window.innerHeight - yPos - 75))
          if (yPos < (window.innerHeight / 2))
            ...
            if ($("#hoverPreview").height() > (yPos - 60)) // preview is too tall for window
              $("#hoverPreview")
                .css("top", "15px")
                .children()
                .css("height", `${yPos - yOffset + 16 - 45}px`);
            else // preview will fit in window if top is used
